-- 套餐维表
drop table temp_hlw_dim_szjt_1yuan_priv_2022;
create table temp_hlw_dim_szjt_1yuan_priv_2022(
    priv_type varchar(20),
    priv_name varchar(50),
    priv_id varchar(20)
);

-- 全市通信客户明细
drop table temp_hlw_szjt_1yuan_shouru_tongx;  -- 字段明细见93行
...
(
    select distinct a.product_no,a.user_id,b.county_name
    from cqbassdb.dw_product_outetype_dt_20211231 a    -- 去年拍照用户
    left join cqbassdb.dim_pub_county_new b on a.county_id=b.county_id
    where a.active=1 and substr(a.gra_product_attr_id1,5,1)=1
) t1 -- 全市存量客户
inner join 
