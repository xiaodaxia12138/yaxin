...
left join temp_hlw_5g_tb_sale_map_info_qyrh t5 on t1.product_no=t5.product_no             -- 权益融合
left join
(
...
select t1.user_id,t1.product_no,t1.is_xy,t1.is_bxl,t1.is_5gzd,t1.is_jt,t1.is_qyrh,t1.is_dy68,t1.is_18_68,t1.is_xy18,t1.is_fk,
        t2.5gtb_cnt,t10.cnt,
...
select  case when t2.is_xy=1 then '校园'
             when t2.is_xy=0 and t2.is_bxl=1 then '不限量'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=1 then '5G终端'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=0 and t2.is_jt=1 then '家庭'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=0 and t2.is_jt=0 and t2.is_qyrh=1 then '权益融合'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=0 and t2.is_jt=0 and t2.is_qyrh=0 and t2.is_dy68=1 then '大于68' 
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=0 and t2.is_jt=0 and t2.is_qyrh=0 and t2.is_dy68=0 and t2.is_18_68=1 then '18-68之间'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=0 and t2.is_jt=0 and t2.is_qyrh=0 and t2.is_dy68=0 and t2.is_18_68=0 and t2.is_xy18=1 then '小于18'
        end as class_tc,
...
from temp_hlw_5g_tb_sale_map_info_02 t2
left join 
(
    select distinct user_id
    from cqbassdb.dw_product_outetype_dt_20211231
...
group by case when t2.is_xy=1 then '校园'
             when t2.is_xy=0 and t2.is_bxl=1 then '不限量'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=1 then '5G终端'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=0 and t2.is_jt=1 then '家庭'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=0 and t2.is_jt=0 and t2.is_qyrh=1 then '权益融合'
...
-- 插入副卡分类
insert into temp_hlw_5g_tb_sale_map_info_03
select  case when t2.is_fk=1 then '副卡' end as class_tc,
        count(t2.user_id) as value1,
        sum(t2.is_5gdd) as value2,
        count(t2.tb_cnt)/count(t2.user_id)as value3,
        sum(is_yyx) as value4,
...
where t2.is_fk=1
group by case when t2.is_fk=1 then '副卡' end 
;


-- 拍照用户
drop table temp_hlw_5g_tb_sale_map_info_pz_01;
create table temp_hlw_5g_tb_sale_map_info_pz_01
(
    user_id varchar(100),
    product_no varchar(100),
    privsetid varchar(100),
...
insert into temp_hlw_5g_tb_sale_map_info_pz_01
select  t1.user_id,t1.product_no,
        ' ',
        case when t2.user_id is not null then 1 else 0 end as is_xy,
        case when t2_1.user_id is not null then 1 else 0 end as is_bxl,
        ...
        case when t8.user_id is not null then 1 else 0 end as is_xy18,
        case when t9.product_no is not null then 1 else 0 end as is_fk 
from cqbassdb.dw_product_outetype_dt_20211231 t1 
left join 
(
    select distinct user_id
    from cqbassdb.dw_user_useage_privilege_dt_20211231
    where privsetid in (
    'gl_hkqcbtc','gl_hkqcbtk','gl_ydhk_19A_qyhy12','gl_ydhk_19A_qyhy6','gl_ydhk_19A_zkhy12','gl_ydhk_19A_zkhy6','gl_ydhk_19atc','gl_ydhk_19atk','gl_ydhk_19B_qyhy12',
    'gl_ydhk_19B_qyhy6','gl_ydhk_19B_zkhy12','gl_ydhk_19B_zkhy6','gl_ydhk_19btc','gl_ydhk_19btk','gl_ydhk_19C_qyhy12','gl_ydhk_19C_qyhy6','gl_ydhk_19C_zkhy12','gl_ydhk_19C_zkhy6',
    'gl_ydhk_19ctc','gl_ydhk_19ctk','gl_ydhk_19D_qyhy12','gl_ydhk_19D_qyhy6','gl_ydhk_19D_zkhy12','gl_ydhk_19D_zkhy6','gl_ydhk_19dtc','gl_ydhk_19dtk',
    ...
    and  nvl(enddate,'2050-01-01') >'2021-05-31' and enddate is null
) t2 on t1.user_id=t2.user_id           
left join 
(
    select distinct user_id
...
(
    select distinct a.user_id,product_no
    from cqbassdb.dw_user_imei_dt_20211231 a
    inner join cqbassdb.dim_pub_5g_terminfo_tac b on a.tac=b.tac
) t3 on t1.user_id = t3.user_id                    -- 5G终端
left join cqbassdb.dw_znjf_cust_zhudinet_info_outside_dm_20211231 t4 on t1.user_id=t4.user_id         -- 家庭用户
left join temp_hlw_5g_tb_sale_map_info_qyrh t5 on t1.product_no=t5.product_no             -- 权益融合
left join
(
    select distinct user_id from cqbassdb.dw_user_nowuse_privilege_dt_20211231 where total_value >= 68
) t6 on t1.user_id=t6.user_id              -- 大于68
left join
(
    select distinct user_id from cqbassdb.dw_user_nowuse_privilege_dt_20211231 where total_value between 18 and 68 
) t7 on t1.user_id=t7.user_id     
left join           -- 18-68
(
    select distinct user_id,'1'  from cqbassdb.dw_user_nowuse_privilege_dt_20211231 where total_value <= 18
) t8 on t1.user_id=t8.user_id               -- 小于18
left join 
(
    select distinct product_no
    from cqbassdb.dw_zzqs_user_sxk_detail_dt_20211231 
    where is_zk_priv='否'
) t9 on t1.product_no=t9.product_no           -- 副卡
where t1.active=1 and substr(t1.gra_product_attr_id1,5,1)=1
;

drop table temp_hlw_5g_tb_sale_map_info_pz_02;
create table temp_hlw_5g_tb_sale_map_info_pz_02
(
    user_id varchar(100),
    product_no varchar(100),
    is_xy int,       -- 是否校园
...
insert into temp_hlw_5g_tb_sale_map_info_pz_02
select t1.user_id,t1.product_no,t1.is_xy,t1.is_bxl,t1.is_5gzd,t1.is_jt,t1.is_qyrh,t1.is_dy68,t1.is_18_68,t1.is_xy18,t1.is_fk,
        t2.5gtb_cnt,t10.cnt,
        case when   t3.user_id is not null or
                    t4.user_id is null or
                    ...
                    t9.user_id is not null then 0 else 1 end as is_yyx,
        case when t2.user_id is not null then 1 else 0 end as is_5gdd 
from temp_hlw_5g_tb_sale_map_info_pz_01 t1
left join 
(
    select distinct user_id,count(1) as 5gtb_cnt from cqbassdb.dw_5g_privilege_user_dt_20211231 where typename in ('5G叠加包','5G套餐')
    group by user_id
) t2 on t1.user_id=t2.user_id
left join 
(  
...
(
    ...
    from 
    (
        select distinct user_id from cqbassdb.DW_ZZQS_USER_LLGX_DT_20211231
        where  is_llgx=1 
        union  all
        select distinct user_id1 as user_id from cqbassdb.DW_ZZQS_USER_LLGX_DT_20211231
        where  is_llgx=1 
        union all
        select distinct user_id2 as user_id from cqbassdb.DW_ZZQS_USER_LLGX_DT_20211231
        where  is_llgx=1 
        union all
        select distinct user_id3 as user_id from cqbassdb.DW_ZZQS_USER_LLGX_DT_20211231
        where  is_llgx=1 
        union all
        select distinct user_id4 as user_id from cqbassdb.DW_ZZQS_USER_LLGX_DT_20211231
        where  is_llgx=1 
    ) a 
) t5 on t1.user_id=t5.user_id
left join 
(
    select distinct product_no
    from cqbassdb.dw_zzqs_user_conn_qianyue_dt_20211231
    where age >= 60
) t6 on t1.product_no=t6.product_no
left join 
(
    select distinct user_id
    from cqbassdb.dw_user_charges_useage_mm_202105
    where usageratio <= 0.3 or dateratio <=0.3
) t7 on t1.user_id=t7.user_id
left join 
(
    select distinct user_id
    from cqbassdb.dwb_imei_double_card_main_type_dt_20211231 
    where product_no is not null and card_type in ('移动移动','电信移动','联通移动')
) t8 on t1.user_id=t8.user_id
left join
(
    select distinct user_id 
    from cqbassdb.dw_zzqs_5g_tb_detail_dt_20211231
    where userstatus_name not in ('临时生成资料','临时资料交费开机','回退') 
    and (enddate>='2021-06-01' or enddate is null) 
    and operate_date between '2021-05-01' and '2021-05-31' 
    and priv_type not in ('副卡','升舱包')
) t9 on t1.user_id=t9.user_id
left join
(
    select distinct user_id,count(1) as cnt from cqbassdb.dw_zzqs_5g_tb_detail_dt_20211231
    where userstatus_name not in ('临时生成资料','临时资料交费开机','回退')  
    and operate_date between '2021-01-01' and '2021-05-31' 
    and (enddate>='2021-06-01' or enddate is null) 
    and priv_type not in ('副卡','升舱包')
    and priv_type='5G叠加包'
    group by user_id
...
;
-- 插入非副卡分类
drop table temp_hlw_5g_tb_sale_map_info_pz_03;
create table temp_hlw_5g_tb_sale_map_info_pz_03(
    class_tc varchar(20),     -- 套餐分类
    value1 int,    -- 通信用户
    value2 int,    -- 5G到达
    value3 decimal(20,4),    -- 5G套包渗透率
...
insert into temp_hlw_5g_tb_sale_map_info_pz_03 
select  case when t2.is_xy=1 then '校园'
             when t2.is_xy=0 and t2.is_bxl=1 then '不限量'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=1 then '5G终端'
             when t2.is_xy=0 and t2.is_bxl=0 and t2.is_5gzd=0 and t2.is_jt=1 then '家庭'
        ...
        sum(case when t4.user_id is not null then t2.is_5gdd end)as value13,
        sum(case when t4.user_id is not null then t2.is_5gdd end)/count(t4.user_id) as value14
from temp_hlw_5g_tb_sale_map_info_pz_02 t2
left join 
(
    select distinct user_id
    from  cqbassdb.dw_product_outetype_dt_20211231 where active=1 and substr(gra_product_attr_id1,5,1)=1
)  t3 on t2.user_id=t3.user_id
left join 
(
    select distinct user_id
...
select  t1.class_tc,
        ...
        t2.value10 ,
        t2.value11 ,
        t2.value9/max(t2.value7) over () ,
        t1.value12 ,
        t1.value13 ,
        t1.value14 ,
        t1.value13/max(t1.value12) over ()
from temp_hlw_5g_tb_sale_map_info_03 t1
left join temp_hlw_5g_tb_sale_map_info_pz_03 t2 on t1.class_tc=t2.class_tc
;
